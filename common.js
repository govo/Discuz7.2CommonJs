...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
	...
	var handleResult = function() {
		...
		if(true){
			/*主要问题在于部分浏览器(Chrome和IE9)无法获取或获取错误（Chrome下获取不完全）iframe的内容而造成BUG*/
			if(BrowserDetect.browser=="Chrome"){
				$(ajaxframeid).src='javascript:void((function(){var xmlNodes=document.getElementsByTagName("root")[0].childNodes,l=xmlNodes.length,s="";for(var i = 0;i<l;i++){ s+= xmlNodes[i].nodeValue;}window.parent.ajaxPostContent=s;})())';
				s=window.ajaxPostContent;
				ajaxframe.loading = 0;
				$('append_parent').removeChild(ajaxframe);
				ajaxframe=null;
			}else if(BrowserDetect.browser=="Explorer" && s==""){
				$(ajaxframeid).src='javascript:void((function(){window.parent.ajaxPostContent=document.childNodes[0].innerText;  })())';
				s=window.ajaxPostContent.replace(/^((\n|.)*?)<!\[CDATA\[/i,"").replace(/\]\]>(\n|.)*?$/i,"");
				ajaxframe.loading = 0;
				$('append_parent').removeChild(ajaxframe);
				ajaxframe=null;
			}
